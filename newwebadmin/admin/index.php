...
if($_GET['did']){
    if($_GET['did']==1) show_msg("超级管理员不可删除",4,'index.php');
    mysql_query("DELETE FROM `web_admin` WHERE `id`='{$_GET['did']}'");
    show_msg("删除成功",4,'index.php');
}

$this_page_name = 'index.php';
$keyword=trim(rp($_REQUEST['keyword']));
$sql_t='SELECT * FROM `web_admin` WHERE 1=1';
if ($action=='search') $sql_t .= " AND `admin_user` LIKE '%".mysql_escape_string($keyword)."%'";
...
<div class="qbt_omnissguai">
    ...
    <tr class="qbt_biaoshiwocunzai">
      ...
      <td align="center"><strong>所属角色</strong></td>
  ...
  ?>
    ...
    <tr>
      ...
      <td <?php if($i_line%2==0){ ?>bgcolor="#fcfcfc"<?php } ?> align="center"><?php echo $rs['admin_user']; ?></td>

      <td <?php if($i_line%2==0){ ?>bgcolor="#fcfcfc"<?php } ?> align="center"><?php echo mysql_result(mysql_query("SELECT `role_name` FROM `web_admin_role` WHERE `id`='{$rs['role_id']}'"), 0); ?></td>

      <td align="center" <?php if($i_line%2==0){ ?>bgcolor="#fcfcfc"<?php } ?>>
        <?php if(($rs['id']>1 || $a_user_id==1) && check_admin_action("edit")){ ?><button class="button button_white" onClick="location.href='changepass.php?id=<?php echo $rs['id']; ?>';return false;">修改</button><?php } ?>
        <?php if($rs['id']>1 && $rs['id']!=$a_user_id && check_admin_action("del")){ ?>
        <button class="button button_white" onClick="if(confirm('确认要删除该用户吗？')){ location.href='?did=<?php echo $rs['id']; ?>'; } return false;">删除</button>
        <?php } ?>
      </td>
